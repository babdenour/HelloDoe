...
export default class CandidateList extends Vue implements CandidateListProps {
  ...
  getCandidateStatus(candidate: Candidate): string {
    if (candidate.status === CandidateStatus.UNLOCKED) {
      return CandidateStatus.UNLOCKED;
    } else if (candidate.status === CandidateStatus.FAVORITE) {
      return CandidateStatus.FAVORITE;
    } else if (candidate.status === CandidateStatus.PRESELECTED) {
      return CandidateStatus.PRESELECTED;
    } else if (candidate.status === CandidateStatus.OTHER) {
  ...
  emitSetCandidateFavorite(candidateId: string): void {
    ...
    if (candidate.status === CandidateStatus.FAVORITE) {
      this.$emit(EmittedEvents.SET_CANDIDATE_FAVORITE, candidateId, false);
    } else if (candidate.status === CandidateStatus.PRESELECTED || CandidateStatus.OTHER) {
      this.$emit(EmittedEvents.SET_CANDIDATE_FAVORITE, candidateId, true);
    }
  }
}
