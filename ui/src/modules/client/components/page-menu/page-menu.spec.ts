...
describe('MenuPage', () => {
  ...
  describe('when translate', () => {
    it('should translate title my candidate', () => {
      ...
      expect(title.props('title')).toBe('pages.client.candidates-page.menu-button.my-candidates');
    });

    it('should translate title tutorial dashboard', () => {
      const title = find(wrapper, MENU_BUTTON_TITLE_DASHBOARD_TUTORIAL);

      expect(title.props('title')).toBe('pages.client.candidates-page.menu-button.dashboard-tutorial');
    });

    it('should translate title recruitment process', () => {
      const title = find(wrapper, MENU_BUTTON_TITLE_RECRUITMENT_PROCESS);

      expect(title.props('title')).toBe('pages.client.candidates-page.menu-button.recruit-process');
    });

    it('should translate title FAQ', () => {
      const title = find(wrapper, CLIENT_FAQ_PAGE);

      expect(title.props('title')).toBe('pages.client.candidates-page.menu-button.faq');
    });
  });
