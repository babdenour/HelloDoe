...
@Injectable()
export class MissionSetFavoriteDoerUseCase {
  constructor(
  ...
  public async run(missionId: string, doerId: string, isFavorite: boolean): Promise<void> {
    await this.validateCanRunUseCase(missionId, doerId);

    if (isFavorite) {
      await this.candidateSvc.addToFavoriteForMission(missionId, doerId);
    } else {
      await this.candidateSvc.removeFromFavoriteForMission(missionId, doerId);
    }
  }
